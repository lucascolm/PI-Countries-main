...
router.get('/',async(req,res)=>{
    ...
    try {
        ...
        if(name){
           const byNameCountries= await Country.findAll({where:{
            name:{
                [Op.iLike] : `%${name}%`
            }
           }})
           return byNameCountries.length>0?res.status(201).json(byNameCountries):res.status(201).send('no encontro paises')
        }else{
            //const countries =  await getCountry();
...
router.get('/:id',async (req,res)=>{
    ...
    try {
        const countries =  await getCountry();
    const byIdCountries= await Country.findByPk(id.toUpperCase(),{include:[Activity]})
    if(byIdCountries){
