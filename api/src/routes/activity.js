...
router.post("/", async (req, res) => {
    ...
    if (!name || !difficulty || !season || !paises)return res.status(404).send("datos faltantes");
    ...
    try {
      ...
      if (paisMatch.length !== 0) {
        ...
        return res.status(400).send("la actividad ya existe y los paies ya estan vinculados");
      }
    }
